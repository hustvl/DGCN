...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes):
        ...
        self.layer3 = self._make_layer(block, 256, layers[2], stride=1, dilation=2)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=1, dilation=4)
        self.layer5 = self._make_pred_layer(Classifier_Module, [12],[12],num_classes)
        # self.layer5 = self._make_pred_layer(Classifier_Module, [6],[6],num_classes)
        # self.layer5 = self._make_pred_layer(Classifier_Module, [6,12,18,24],[6,12,18,24],num_classes)
        # self.linear = nn.Sequential(
        #     nn.AvgPool2d(kernel_size=41, stride=1),
        #     nn.Linear(2048*1*1,20,bias=False))
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                n = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
                m.weight.data.normal_(0, 0.01)
    ...
    def forward(self, x):
        ...
        x = self.layer4(x)
        #feature = x
        #linear_x = self.linear[0](x)
        #linear_x = linear_x[:, :, 0, 0]
        #score = self.linear[1](linear_x)
        x = self.layer5(x)

        return feature1,feature2,x#,feature
